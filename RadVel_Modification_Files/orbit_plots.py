...
class MultipanelPlot(object):
    ...
    def __init__(self, post, saveplot=None, epoch=2450000, yscale_auto=False, yscale_sigma=3.0,
                 phase_nrows=None, phase_ncols=None, uparams=None, telfmts={}, legend=True, telnames=None,
        ...
        if self.telnames is not None:
            self.rverr = self.post.likelihood.chromatic_errorbars(self.post.likelihood.telvec)
        else:
...
#Added by CB 4/18/2022
class ChromaticGPMultipanelPlot(MultipanelPlot):
    """
    Class to handle the creation of RV multipanel plots for posteriors fitted
    using the special Chromatic Gaussian Processes.
    ...
    Args:
        ...
        subtract_orbit_model (bool, optional): if True, subtract the best-fit
            ...
            noise in the data. Default: False.
        status (ConfigParser): (optional) result of radvel.driver.load_status on the .stat status file
        telnames: Array listing the name of each instrument

    """
    def __init__(self, post, saveplot=None, epoch=2450000, yscale_auto=False, yscale_sigma=3.0,telnames=[],
                 phase_nrows=None, phase_ncols=None, uparams=None, rv_phase_space=0.08, telfmts={},
                 legend=True,
                 phase_limits=[], nobin=False, phasetext_size='large',  figwidth=7.5, fit_linewidth=2.0,
                 set_xlim=None, text_size=9, legend_kwargs=dict(loc='best'), subtract_gp_mean_model=False,
        ...
        super(ChromaticGPMultipanelPlot, self).__init__(
            post, saveplot=saveplot, epoch=epoch, yscale_auto=yscale_auto,
            yscale_sigma=yscale_sigma, telnames=telnames, phase_nrows=phase_nrows, phase_ncols=phase_ncols,
            uparams=uparams, rv_phase_space=rv_phase_space, telfmts=telfmts, legend=legend,
            phase_limits=phase_limits, nobin=nobin, phasetext_size=phasetext_size,
            figwidth=figwidth, fit_linewidth=fit_linewidth, set_xlim=set_xlim, text_size=text_size,
            legend_kwargs=legend_kwargs
        ...
        is_gp = False
        for like in self.like_list:
            if isinstance(like, radvel.likelihood.ChromaticLikelihood):
                is_gp = True
                break
            else:
                pass
        assert is_gp, "This class requires at least one ChromaticLikelihood Object. Are you sure you wouldn't be happier using GPMultiPanelPlot?"

    def plot_gp_like(self, like, inst, orbit_model4data, ci):
        """
        Plot a single Gaussian Process Likleihood object in the current Axes,
        including Gaussian Process uncertainty bands.

        Args:
            like (radvel.ChromaticLikelihood): radvel.ChromaticLikelihood object. The model
                plotted will be generated from `like.params`.
            orbit_model4data (numpy array):
            ci (int): index to use when choosing a color to plot from
                radvel.plot.default_colors. This is only used if the
        ...
        if isinstance(like, radvel.likelihood.ChromaticLikelihood):
            ...
            xpred = np.linspace(np.min(like.x), np.max(like.x), num=int(3e3))
            gpmu, stddev = like.predict(xpred, inst)

            if self.subtract_orbit_model:
                gp_orbit_model = np.zeros(xpred.shape)
            elif inst == 'ffprime':
                gp_orbit_model = np.zeros(xpred.shape) #ffprime has no planet contribution
            else:
                gp_orbit_model = self.model(xpred)
            ...
            if self.subtract_gp_mean_model:
                ...
                gp_mean4data, _ = like.predict(like.x, inst)
                indx = np.where(self.post.likelihood.telvec == inst)
                orbit_model4data[indx] += gp_mean4data[indx]

            if inst not in self.telfmts and inst in plot.telfmts_default:
                color = plot.telfmts_default[inst]['color']
            if inst in self.telfmts:
                color = self.telfmts[inst]['color']
            if inst not in self.telfmts and inst not in plot.telfmts_default:
                color = plot.default_colors[ci]
                ci += 1
            ...
            ax.plot(xpred, gpmu+gp_orbit_model, '-', rasterized=False, lw=0.1, color=color)
    ...
    def plot_timeseries(self):
        ...
        ax = pl.gca()
        like = self.like_list[0] #should all be in one object
        ...
        for i in range(len(like.telvec)):
            if like.telvec[i] == 'ffprime':
                orbit_model4data[i] = 0 #the ffprime data should not have an RV signal
            else:
                continue


        ci = 0
        for inst in self.telnames:
            ci = self.plot_gp_like(like, inst, orbit_model4data, ci)
        ...
        plot.mtelplot(
            # data = residuals + model
            #self.plttimes, self.rawresid+orbit_model4data, self.rverr,
            self.plttimes, self.post.likelihood.y - self.post.likelihood.like_list[0].gammas_total, self.rverr,
            self.post.likelihood.telvec, ax, telfmts=self.telfmts
        )
    ...
    def plot_multipanel(self, nophase=False):
        ...
        if not self.plot_likelihoods_separately:
            super(ChromaticGPMultipanelPlot, self).plot_multipanel()
        else:
            if nophase:
                scalefactor = 1
            else:
            ...
            n_inst = len(self.telnames)
            like = self.like_list[0] #Chromatic Likelihood puts all likelihoods into one object
            figheight = self.ax_rv_height*(n_inst+0.5) + self.ax_phase_height * scalefactor
            ...
            hrs = np.zeros(n_inst+1) + 1.
            hrs[-1] = 0.5
            gs_rv = gridspec.GridSpec(n_inst+1, 1, height_ratios=hrs)

            divide = 1 - self.ax_rv_height*n_inst / figheight
            gs_rv.update(left=0.12, right=0.93, top=0.93,
                         bottom=divide+self.rv_phase_space*0.5, hspace=0.0)
            ...
            for inst in self.telnames:
                ...
                if self.subtract_orbit_model:
                    ...
                    orbit_model4data = self.rvmod
                if inst == 'ffprime':
                    orbit_model4data = np.zeros(len(self.rvmod))

                self.plot_gp_like(like, inst, orbit_model4data, ci)
            ...
            if not nophase:
                ...
                for i in range(self.num_planets):
                    ...
                    pl.sca(ax_phase)
                    self.plot_phasefold(pltletter, i+1, telvec=self.telvec)
                    pltletter += 1
